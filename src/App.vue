  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">目标星座</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">占卜诉求</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">十二星座运势与塔罗疗愈解压指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 十二星座运势与塔罗专家');
...
const metricsList = [
  {
    "key": "astrologicalChartPrecision",
    "label": "星盘/塔罗解读精准度"
  },
  {
    "key": "psychologicalHealingWarmth",
    "label": "心理抚慰温暖度"
  },
  {
    "key": "personalityInsightDepth",
    "label": "性格特质洞察深度"
  },
  {
    "key": "actionAdvicePositivity",
    "label": "积极行动建议性"
  },
  {
    "key": "cringeFreeFunIndex",
    "label": "趣味解压娱乐分"
  }
];
const guideRules = [
  {
    "title": "给予积极心理暗示",
    "advice": "强调“命运掌握在自己手中”，占卜与星座是帮助自我觉察的镜子，多给正面鼓励",
    "avoid": "切忌散播恐慌或给出消极绝对的断言"
  },
  {
    "title": "结合生活可落地建议",
    "advice": "除了能量解读，提供明确的具体行动（如“今天适合整理房间”、“主动给对方发一句问候”），增加落地性",
    "avoid": "避免大段虚无缥缈的玄学抽象词汇"
  },
  {
    "title": "尊重个性差异与包容",
    "advice": "分析星座性格时强调特质的两面性（如“严谨也是细致”），帮助用户接纳真实的自己",
    "avoid": "严禁对特定星座进行刻板印象标签抹黑"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[XINGZUO_SCORES\][\s\S]*?\[\/XINGZUO_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[XINGZUO_SCORES\](.*?)\[\/XINGZUO_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('xingzo_history_records');
...
const saveHistory = () => {
  localStorage.setItem('xingzo_history_records', JSON.stringify(historyList.value));
};
